...
export default function Button({href, to, children, variant = "primary", onClick, download, showArrow = true}) {
    ...
    if (href) return <a href={href} onClick={onClick} target={opensNewTab ? "_blank" : undefined} rel={opensNewTab ? "noreferrer" : undefined} download={download} className={cn("group", base, styles)} {...buttonProps}>{children}{showArrow && arrow}</a>;
